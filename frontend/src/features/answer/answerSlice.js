...
export const getAllAnswers = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      const res = await axios.get("/answer/getAll");
      console.log("ans front", res.data);
      return res.data;
    } catch (err) {
      return rejectWithValue(err.response?.data || err.message);
    }
  ...
  async ({ questionId }, { rejectWithValue }) => {
    try {
      const res = await axios.get(
        `/answer/getAllAnswersByQuestionId/${questionId}`
      );
      return res.data;
    } catch (err) {
      return rejectWithValue(err.response?.data || err.message);
...
export const checkHasFeedbackForTherapySession = createAsyncThunk(
  ...
  async ({ therapySessionId, employeeId }, { rejectWithValue }) => {
    try {
      const res = await axios.get(
        `/answer/checkFeedbackForTherapySession/${therapySessionId}/${employeeId}`
      );
      return {
        key: `therapy-${therapySessionId}`,
        hasFeedback: res.data.hasFeedback,
      };
    } catch (err) {
      return rejectWithValue(err.response?.data || err.message);
    }
...
export const checkHasFeedbackForEvent = createAsyncThunk(
  ...
  async ({ eventId, employeeId }, { rejectWithValue }) => {
    try {
      const res = await axios.get(
        `/answer/checkFeedbackForEvent/${eventId}/${employeeId}`
      );
